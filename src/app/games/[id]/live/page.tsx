...
type Stat = {
  ...
  reb: number; ast: number; tov: number; stl: number; blk: number; pf: number;
};
...
const emptyStat = (): Stat => ({
  ...
  fg3m: 0, fg3a: 0,
  ftm: 0, fta: 0,
  reb: 0, ast: 0, tov: 0, stl: 0, blk: 0, pf: 0,
});
...
function applyEventToStat(stat: Stat, e: EventRow) {
  switch (e.event_type) {
    case "FG2_MAKE": stat.pts += 2; stat.fg2m += 1; stat.fg2a += 1; break;
    case "FG2_MISS": stat.fg2a += 1; break;

    case "FG3_MAKE": stat.pts += 3; stat.fg3m += 1; stat.fg3a += 1; break;
    case "FG3_MISS": stat.fg3a += 1; break;

    case "FT_MAKE": stat.pts += 1; stat.ftm += 1; stat.fta += 1; break;
    case "FT_MISS": stat.fta += 1; break;

    case "REB": stat.reb += 1; break;
    case "AST": stat.ast += 1; break;
    case "TOV": stat.tov += 1; break;
    case "STL": stat.stl += 1; break;
    case "BLK": stat.blk += 1; break;
    case "PF": stat.pf += 1; break;
  }
}
...
export default function LivePage() {
    ...
    (async () => {
      ...
      if (c.error) {
        ...
        if (c2.error) setMsg(c2.error.message);
          ...
          setClock(c2.data as any);
          setQuarter((c2.data as any)?.quarter ?? 1);
        }
      } else {
        setClock(c.data as any);
        setQuarter((c.data as any)?.quarter ?? 1);
      }
    })();
  }, [gameId]);
  ...
  async function record(eventType: string) {
    ...
    if (side === "home" && !selectedPlayer) return setMsg("請先選擇球員");
...
const payload = {
  ...
  created_by: auth.user.id, // ✅ 加這個最關鍵
};
  ...
  async function undoLast() {
    ...
    // 用 activeEvents 找最後一筆（避免被 Undo 的混進來）
    ...
    // 立即更新 UI（不等 realtime）
    setEvents((prev) => prev.map((x) => (x.id === last.id ? { ...x, is_undone: true, undone_at: new Date().toISOString() } : x)));

    setMsg("↩️ 已撤銷最後一筆");
  }

  // ===== update clock (optimistic) =====
  ...
  useEffect(() => {
    ...
    if (writeBackRef.current) { clearInterval(writeBackRef.current); writeBackRef.current = null; }
  ...
  const btnGreen: React.CSSProperties = { ...btnBase, background: "#22c55e", color: "#052e12", fontWeight: 800 };
  ...
  return (
    <main style={{ padding: 20 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <Link href="/games/new">建立新比賽</Link>
      </div>
      ...
      >
        ...
        <button style={btnGray} onClick={() => updateClock({ seconds_left: Math.max(0, (clock?.seconds_left ?? 0) + 60) })}>+1分</button>
        <button style={btnGray} onClick={() => updateClock({ seconds_left: Math.max(0, (clock?.seconds_left ?? 0) - 60) })}>-1分</button>
        <button style={btnGray} onClick={() => updateClock({ seconds_left: Math.max(0, (clock?.seconds_left ?? 0) + 10) })}>+10秒</button>
        <button style={btnGray} onClick={() => updateClock({ seconds_left: Math.max(0, (clock?.seconds_left ?? 0) - 10) })}>-10秒</button>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
        ...
        {/* ✅ Undo 放旁邊，手感最好 */}
        <button style={btnGray} onClick={undoLast}>Undo</button>
      </div>
      ...
      {/* ===== actions (把「事件」改成「快速記錄」更好看) ===== */}
      <h3>快速記錄</h3>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 18 }}>
        <button style={btnGray} onClick={() => record("FG2_MAKE")}>+2</button>
        <button style={btnGray} onClick={() => record("FG3_MAKE")}>+3</button>
        <button style={btnGray} onClick={() => record("FT_MAKE")}>罰進</button>

        <button style={btnGray} onClick={() => record("FG2_MISS")}>2分沒進</button>
        <button style={btnGray} onClick={() => record("FG3_MISS")}>3分沒進</button>
        <button style={btnGray} onClick={() => record("FT_MISS")}>罰失</button>

        <button style={btnGray} onClick={() => record("REB")}>籃板</button>
        <button style={btnGray} onClick={() => record("AST")}>助攻</button>
        <button style={btnGray} onClick={() => record("STL")}>抄截</button>

        <button style={btnGray} onClick={() => record("BLK")}>火鍋</button>
        <button style={btnGray} onClick={() => record("TOV")}>失誤</button>
        <button style={btnGray} onClick={() => record("PF")}>犯規</button>
      </div>
          ...
          <thead style={{ background: "#111" }}>
            <tr>
              {["球員", "PTS", "2PT", "3PT", "FT", "REB", "AST", "TO", "STL", "BLK", "PF"].map((h) => (
                <th key={h} style={{ textAlign: "left", padding: 10, borderBottom: "1px solid #222", color: "#ddd" }}>
                  {h}
                </th>
              ))}
            </tr>
              ...
              .map((p) => {
                ...
                return (
                  <tr key={p.id} style={{ background: isSelected ? "rgba(34,197,94,0.12)" : "transparent" }}>
                    ...
                    <td style={{ padding: 10, borderBottom: "1px solid #1f1f1f", color: "white", fontWeight: 900 }}>{st.pts}</td>
